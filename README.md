...
| `tls_cert_path` | String or `nil` | `nil` | Client certificate file path for mTLS. Configure with `tls_key_path`. Symlinks are rejected. |
| `tls_key_path` | String or `nil` | `nil` | Client private key file path for mTLS. Configure with `tls_cert_path`. Symlinks are rejected. |
| `tls_server_root_ca_cert_path` | String or `nil` | `nil` | Optional root CA certificate file path for self-hosted Temporal TLS. Symlinks are rejected. |
...
| `ACTIVEJOB_TEMPORAL_DEAD_LETTER_AUTO_DISCARD_AFTER_SECONDS` | No | Seconds to keep pending dead letter workflows before auto-discarding. | `2592000` |
